class IssueTemplatesController < ApplicationController
  ...
  def index
    ...
    @project.trackers.each do |tracker|
      templates = IssueTemplate.where('tracker_id = ?', tracker.id).order('position')
      if templates.any?
        @template_map[tracker] = templates
      end
    end

    @issue_templates = IssueTemplate.find(:all, :order => 'position')
  ...
  def new
    ...
    @issue_template = IssueTemplate.new(:author => @user, :tracker => @tracker)
    if request.post?
  ...
  # update pulldown
  def set_pulldown
    issue_templates = IssueTemplate.where('tracker_id = ? AND enabled = ?',
                                            @tracker.id, true).order('position')
    @grouped_options = []
    group = []

    @default_template = IssueTemplate.where('tracker_id = ? AND enabled = ? AND is_default = ?',
                                            @tracker.id, true, true).first
